...
class Statustype {
	...
	public function load($paramId) {
		...
		$stmt = $conn->prepare('CALL usp_statustype_Load(?)');
	...
	private function insert() {
		include(self::getDbSettings());
		$conn = new mysqli($servername, $username, $password, $dbname);
		$stmt = $conn->prepare('CALL usp_statustype_Add(?,?)');
		$arg1 = $this->getName();
	...
	private function update() {
		include(self::getDbSettings());
		$conn = new mysqli($servername, $username, $password, $dbname);
		$stmt = $conn->prepare('CALL usp_statustype_Update(?,?,?)');
		$arg1 = $this->getId();
		$arg2 = $this->getName();
	...
	public static function loadall() {
		include(self::getDbSettings());
		$conn = new mysqli($servername, $username, $password, $dbname);
		$stmt = $conn->prepare('CALL usp_statustype_LoadAll()');
		$stmt->execute();
		...
		if ($result->num_rows > 0) {
			$arr = array();
			while ($row = $result->fetch_assoc()) {
				$statustype = new Statustype($row['Id'],$row['Name'],$row['Description']);
				$arr[] = $statustype;
			}
			return $arr;
		}
	...
	public static function remove($paramId) {
		include(self::getDbSettings());
		$conn = new mysqli($servername, $username, $password, $dbname);
		$stmt = $conn->prepare('CALL usp_statustype_Remove(?)');
		$stmt->bind_param('i', $paramId);
		$stmt->execute();
	}
	...
	public static function search($paramId,$paramName,$paramDescription) {
		include(self::getDbSettings());
		$conn = new mysqli($servername, $username, $password, $dbname);
		$stmt = $conn->prepare('CALL usp_statustype_Search(?,?,?)');
		$arg1 = Statustype::setNullValue($paramId);
		$arg2 = Statustype::setNullValue($paramName);
		$arg3 = Statustype::setNullValue($paramDescription);
		$stmt->bind_param('iss',$arg1,$arg2,$arg3);
		$stmt->execute();
